...
var MvcGrid = (function () {
    ...
    MvcGrid.prototype = {
        ...
        applyFilter: function (column) {
            ...
            var filterValue = $('.mvc-grid-popup:eq(1) > .popup-content > .first-filter:eq(1) > .mvc-grid-input').val();
            var filterName = $('.mvc-grid-popup:eq(1) > .popup-content > .first-filter:eq(0) > .mvc-grid-type').val();

            this.queryAdd(this.prefix + column.name + '-' + filterName, filterValue);
